...
//    dbenv.log_set_config(DB_LOG_AUTO_REMOVE, 1);
    ...
    fDbEnvInit = true;
    fMockDb = false;
     // Check that the number of locks is sufficient (to prevent chain fork possibility, read http://kingcoin.org/may15 for more info)
    u_int32_t nMaxLocks;
    if (!dbenv.get_lk_max_locks(&nMaxLocks))
    {
        ...
        {
            if (nBlocks < 1)
                strMessage = strprintf(_("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a single block. If this limit is reached, KingCoin may stop working."), (unsigned long)nMaxLocks);
            else
                strMessage = strprintf(_("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a common blockchain reorganization. If this limit is reached, KingCoin may stop working."), (unsigned long)nMaxLocks);
