...
{
    CKingCoinAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            scriptPubKey.SetDestination(CKingCoinAddress(rcp.address.toStdString()).Get());
...
{
    OutputDebugStringF("NotifyAddressBookChanged %s %s isMine=%i status=%i\n", CKingCoinAddress(address).ToString().c_str(), label.c_str(), isMine, status);
    QMetaObject::invokeMethod(walletmodel, "updateAddressBook", Qt::QueuedConnection,
                              Q_ARG(QString, QString::fromStdString(CKingCoinAddress(address).ToString())),
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CKingCoinAddress(address).ToString().c_str()].push_back(out);
    }
}
