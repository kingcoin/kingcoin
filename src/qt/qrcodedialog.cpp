...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non KING unit input in lnReqAmount, we generate the URI with KING as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(KingCoinUnits::format(KingCoinUnits::KING, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
