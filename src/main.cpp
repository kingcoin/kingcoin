...
// Copyright (c) 2009-2012 The KingCoin developers
...
unsigned int nStakeMinAge = 60 * 60 * 24 * 8; // minimum age for coin age
unsigned int nStakeMaxAge = 60 * 60 * 24 * 32; // stake age of full weight
unsigned int nStakeTargetSpacing = 1 * 60; // DIFF: 1-minute block spacing
int64 nChainStartTime = 1376913600;//Mon, 19 Aug 2013 12:00:00 GMT
int nCoinbaseMaturity = 98;//mining need 128 confirms
                              ...
                              enum GetMinFee_mode mode, unsigned int nBytes) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 KING if made of 50 KING inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
const unsigned char MinimumN = 10;
const unsigned char MaximumN = 30;

unsigned char CalculateN(int64 nTimestamp) {
    int l = 0;
    ...
    if (n > 255)
        printf( "CalculateN(%lld) - something wrong(n == %d)\n", nTimestamp, n );

    unsigned char N = (unsigned char) n;
    //printf("CalculateN: %d -> %d %d : %d / %d\n", nTimestamp - nChainStartTime, l, s, n, min(max(N, MinimumN), MaximumN));

    return min(max(N, MinimumN), MaximumN);
}

int64 GetProofOfWorkReward(unsigned int nBits, int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    // (nSubsidyLimit / nSubsidy) ** 8 == bnProofOfWorkLimit / bnTarget
    //
    // Human readable form:
    //
    // nSubsidy = 32 / (diff ^ 1/8)
    ...
    while (bnLowerBound + CENT <= bnUpperBound)
    {
        ...
        if (bnMidValue * bnMidValue * bnMidValue * bnMidValue * bnMidValue * bnMidValue * bnMidValue * bnMidValue * bnTargetLimit > bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnTarget)
    ...
    nSubsidy = max(MIN_MINT_PROOF_OF_WORK,min(nSubsidy, MAX_MINT_PROOF_OF_WORK));

    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfWorkReward() : create=%s nBits=0x%08x nHeight=%d nSubsidy=%"PRI64d"\n", FormatMoney(nSubsidy).c_str(), nBits, nHeight, nSubsidy);

    return nSubsidy + nFees;//fees should be add to next block
}

// ppcoin: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge)
{
    static int64 nRewardCoinYear = 0.32 * CENT;  // creation amount per coin-year
...
static const int64 nTargetSpacingWorkMax = 8 * nStakeTargetSpacing; // 8 minutes
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in KingCoin
    bool fStrictPayToScriptHash = true; // Always active in KingCoin
    ...
    // ppcoin: fees are not collected by miners as in kingcoin
    ...
    if (vtx[0].GetValueOut() > GetProofOfWorkReward(pindex->nBits,pindex->nHeight, nFees, prevHash))
        return false;
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    printf("pindexNew->nStakeModifierChecksum=%x\n",pindexNew->nStakeModifierChecksum);
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "KingCoin - The Coin control the whole world.";
        CTransaction txNew;
        ...
        block.nNonce   = 446763;//by calc GB
        ...
        assert(block.hashMerkleRoot == uint256("0xd6f41fe7ee1741acbfc0a6f7f21172e0f1678356159a914927b83681a9caba6a"));
...
*/
    ...
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 1000) && !fTestNet && !IsInitialBlockDownload())
    {
        nPriority = 100;
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfWork())
            pblock->vtx[0].vout[0].nValue = GetProofOfWorkReward(pblock->nBits,pindexPrev->nHeight+1, nFees, pindexPrev->GetBlockHash());
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("KingCoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("KingCoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KingCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("KingCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadKingCoinMiner(void* parg);

static bool fGenerateKingCoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void KingCoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("kingcoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateKingCoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateKingCoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running KingCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadKingCoinMiner(void* parg)
{
    ...
    try
    {
        ...
        KingCoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadKingCoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadKingCoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadKingCoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateKingCoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateKingCoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateKingCoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d KingCoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadKingCoinMiner, pwallet))
                printf("Error: NewThread(ThreadKingCoinMiner) failed\n");
            Sleep(10);
        }
    }
}
